...
import FastingHistory from "./FastingHistory";
 ...
 const FastCountdownTimer = (props) => {
  ...
  const get_user_fast = async() => {
    ...
    setCurrentFast(active)
    const history = res.filter(res =>{return res.completed === true})
    setFastDetails(history)
    console.log("active:", currentFast)
    console.log("hist:",fastDetails)
...
if (clicked){
  return(<button onClick={doubleclick}>Get Fasting History</button> );
    ...
    return(
      <button onClick={click}>Get Fasting History</button> 
    )
  }
